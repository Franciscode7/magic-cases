...
const Divider = ({ 
  amberBg = "bg-gray-100", // Fondo de la sección de ARRIBA
  waveColor = "text-white", // Color de la sección de ABAJO (relleno de la onda)
  flip = false 
}) => {
  return (
    <div className={`${amberBg} w-full overflow-hidden leading-[0]`}>
      <svg
        className={`relative block w-full h-[40px] md:h-[80px] ${flip ? 'rotate-180' : ''}`}
        data-name="Layer 1"
